<?php
 ...
 class ProdModel{
    ...
    private $db;
     ...
     function editarProducto($nombre_p,$marca_p,$precio_id,$stock_id,$id_categoria){
        $sentencia = $this->db->prepare('UPDATE INTO producto WHERE id_productos=?');
        $sentencia->execute(array($nombre_p,$marca_p,$precio_id,$stock_id,$id_categoria));
     }


     function eliminarProducto($id){
        $sentencia = $this->db->prepare('DELETE FROM producto WHERE id_producto=$id');
        $sentencia->execute(array($id));
     }
